...
Adafruit_ADXL345_Unified accel = Adafruit_ADXL345_Unified(123456);
...
void updateAccelerometer()
{
  if (simulating)
  {
    smoothX = random(0, 255*measurements);
    smoothY = random(0, 255*measurements);
    smoothZ = 1500;
  ...
  else
  {
    ...
    if (currentMeasurement >= measurements)
    {
      ...
      debug("X: ");
      debug(event.acceleration.x);
      debug("  ");
      debug("Y: ");
      debug(event.acceleration.y);
      debug("  ");
      debug("Z: ");
      debug(event.acceleration.z);
      debug("  ");
      debugln("m/s^2 ");
